...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // salary computation -- each entry's percent is an extra cut of the
      // Final Pool added on top of that IGN's own Initial Computation share
      // (not deducted from anyone else's), so it isn't tied to any one
      // month/pool the way the computation itself is.
      await sql`
