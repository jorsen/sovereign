...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // before this existed (or not yet marked either way). Losing still
      // matters: admins can award bonus points to whoever showed up anyway
      ...
      // One row per attendee awarded a bonus for a lost fight -- tied to the
      // specific event (not just a bare running total) so the award is
      // auditable back to which loss it came from, and re-saving the event
