...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // inserts where no linked batch exists yet for that exact kill, so
      // this is idempotent and never duplicates a batch on a later startup.
      await sql`
        ...
        WHERE (li.sold OR COALESCE(li.sold_quantity, 0) > 0)
          ...
          AND NOT EXISTS (
            ...
            WHERE b.source_attendance_id = li.attendance_id
