...
// Express 4 doesn't catch a promise rejected by an async route handler --
// left alone, a bug like an unexpected DB type error (e.g. a decimal value
// hitting a BIGINT column) throws inside a request with no response ever
// sent back, reading to the client as "just doesn't save" with no error
// shown. Wrapping every handler here once (instead of adding try/catch to
// each of this file's ~80 routes individually) forwards any such error to
// the JSON error handler registered at the bottom of this file instead.
['get', 'post', 'put', 'delete', 'patch'].forEach((method) => {
  const original = app[method].bind(app);
  app[method] = (path, ...handlers) =>
    original(
      path,
      ...handlers.map((h) => (typeof h === 'function' ? (req, res, next) => Promise.resolve(h(req, res, next)).catch(next) : h))
    );
});
...
app.post('/api/growth-submissions', async (req, res) => {
  ...
  // Only IGN is truly required -- an admin filling this in after someone
  // already posted their own /uniongr submission shouldn't have to re-type
  // everything just to nudge one field (e.g. a new lamp level). Anything
  // left blank falls back to whatever that existing record already has;
  // only a genuinely new IGN (no existing record to inherit from) needs
  // every field provided.
  const { rows: existingRows } = await sql`SELECT * FROM growth_submissions WHERE LOWER(ign) = LOWER(${ignTrimmed})`;
  const existing = existingRows[0] || null;

  const classFinal = className && className.trim() ? className.trim() : existing ? existing.class : null;
  if (!classFinal) return res.status(400).json({ error: 'class is required for a new IGN' });

  const guildFinal = guildName || (existing ? existing.guild_name : null);
  if (guildFinal && !GROWTH_SUBMISSION_GUILDS.includes(guildFinal)) {
    return res.status(400).json({ error: `guildName must be one of: ${GROWTH_SUBMISSION_GUILDS.join(', ')}` });
  }
  if (!guildFinal) return res.status(400).json({ error: 'guildName is required for a new IGN' });

  let lampLevelFinal = existing ? existing.lamp_level : null;
  if (lampLevel !== undefined && lampLevel !== null && lampLevel !== '') {
  ...
  if (lampLevelFinal === null || lampLevelFinal === undefined) return res.status(400).json({ error: 'lampLevel is required for a new IGN' });

  let growthRateFinal = existing ? existing.growth_rate : null;
  if (growthRate !== undefined && growthRate !== null && growthRate !== '') {
  ...
  if (growthRateFinal === null || growthRateFinal === undefined) return res.status(400).json({ error: 'growthRate is required for a new IGN' });

  // Optional here (unlike the bot route) -- an admin adding/updating an
  // entry by hand shouldn't need a screenshot on hand just to fix a number.
  let imageBuffer = existing ? existing.image_data : null;
  let imageContentTypeFinal = existing ? existing.image_content_type : null;
  if (imageBase64 && imageContentType && imageContentType.startsWith('image/')) {
    try {
      imageBuffer = Buffer.from(imageBase64, 'base64');
      imageContentTypeFinal = imageContentType;
    } catch {
  ...
  if (isUpdate) {
    ({ rows } = await sql`
      UPDATE growth_submissions SET
        ign = ${ignTrimmed},
        class = ${classFinal},
        guild_name = ${guildFinal},
        lamp_level = ${lampLevelFinal},
        growth_rate = ${growthRateFinal},
        image_data = ${imageBuffer},
        image_content_type = ${imageContentTypeFinal},
    ...
    ({ rows } = await sql`
      ...
      VALUES (${id}, ${crypto.randomUUID()}, ${'manual:' + req.currentUser.id}, ${submitterLabel}, ${ignTrimmed}, ${classFinal}, ${guildFinal}, ${lampLevelFinal}, ${growthRateFinal}, ${imageBuffer}, ${imageContentTypeFinal})
      RETURNING id, ign, class, guild_name, lamp_level, growth_rate, discord_username, image_content_type, created_at, updated_at
    `);
  }
  await logActivity(req, {
    ...
    after: { ign: ignTrimmed, class: classFinal, guildName: guildFinal, lampLevel: lampLevelFinal, growthRate: growthRateFinal },
  });
...
app.get('/api/growth-submissions/:id/image', async (req, res) => {
  ...
  const row = rows[0];
  // No 404 -- a valid submission just made without a screenshot (an admin
  // add/edit, which no longer requires one) has no image at all, not a
  // missing-record error.
  if (!row || !row.image_data) return res.status(204).end();
...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  // Same rank order as the website's own leaderboard, IGN only (no growth
  // rate numbers) -- the bot uses this plus the count to post an
  // admin-facing ranked roster alongside the save confirmation, without a
  // second round-trip to compute it itself.
  const { rows: rankedRows } = await sql`SELECT ign FROM growth_submissions ORDER BY growth_rate DESC NULLS LAST, created_at DESC`;
  res.status(201).json({
    ...serializeGrowthSubmission(rows[0]),
    totalSubmissions: rankedRows.length,
    rankedIgns: rankedRows.map((r) => r.ign),
    isNewSubmission: !isUpdate,
  });
});
...
// A "schedule" is an independent boss-attendance tracker -- its own
// calendar, attendance log, and loot -- sharing the same tables/routes as
// the original World Boss Attendance feature via a `schedule` column,
// instead of duplicating every table/route/UI function for each one.
// Adding a new tracker is just adding an entry here (plus its own boss
// name list) rather than copy-pasting the feature.
const WORLD_BOSS_SCHEDULES = {
  world_boss: { label: 'World Boss', bossNames: WORLD_BOSS_NAMES },
  bf4: { label: 'BF4 Boss', bossNames: ['BF4 Boss'] },
};
function bossNamesForSchedule(schedule) {
  return (WORLD_BOSS_SCHEDULES[schedule] || WORLD_BOSS_SCHEDULES.world_boss).bossNames;
}

function serializeWorldBossEvent(row, attendees, lootItems, bonusRows) {
  return {
    id: row.id,
    schedule: row.schedule || 'world_boss',
    bossName: row.boss_name,
    eventDate: row.event_date,
    createdAt: row.created_at,
    result: row.result || 'pending',
    // A single amount, not per-attendee -- every bonus row on one event is
    // always written with the same points value (see buildBonusPointInserts),
    // so the first row (if any) tells the edit form what to prefill.
    bonusPoints: bonusRows && bonusRows.length ? Number(bonusRows[0].points) : null,
    ...
    lootItems: (lootItems || []).map((l) => ({
      ...
      sold: l.sold === true,
      soldQuantity: l.sold_quantity === null || l.sold_quantity === undefined ? (l.sold ? l.quantity : 0) : Number(l.sold_quantity),
    })),
  };
}
...
// Kept in sync with the same map in public/sovereign/crusade/app.js -- that
// copy only affects *display* in the merged monthly view; this one corrects
// the name before it's ever stored, so a typo'd item doesn't end up as its
// own separate loot_items row on the same kill (which happened for real:
// "...Fragment" x4 and "...Fragmer" x1 sat as two rows on one Balthazard
// kill until someone noticed and it had to be fixed by hand).
...
function normalizeLootItems(lootItems) {
  ...
  const cleaned = lootItems
    .map((l) => {
      const quantity = Number.isFinite(Number(l?.quantity)) && Number(l?.quantity) > 0 ? Math.floor(Number(l.quantity)) : 1;
      const sold = l?.sold === true;
      // Client sends this explicitly once FIFO-matched (see
      // applyFifoSalesToItem); anything else (a brand new row, or one
      // written before this column existed) falls back to the old
      // all-or-nothing meaning of `sold`.
      const soldQuantityRaw = Number(l?.soldQuantity);
      const soldQuantity = Number.isFinite(soldQuantityRaw) ? Math.max(0, Math.min(quantity, soldQuantityRaw)) : sold ? quantity : 0;
      return {
        itemName: canonicalizeItemName(l?.itemName),
        quantity,
        // NUMERIC columns -- decimals allowed (e.g. splitting one sale's
        // total proportionally across several kills), just guarded against
        // garbage input reaching Postgres as a bad parameter.
        crowsValue: l?.crowsValue === '' || l?.crowsValue === null || l?.crowsValue === undefined || !Number.isFinite(Number(l.crowsValue)) ? null : Number(l.crowsValue),
        diamondsValue: l?.diamondsValue === '' || l?.diamondsValue === null || l?.diamondsValue === undefined || !Number.isFinite(Number(l.diamondsValue)) ? null : Number(l.diamondsValue),
        sold: soldQuantity >= quantity,
        soldQuantity,
      };
    })
    .filter((l) => l.itemName);

  // Dedupes same-name rows within this one submission (e.g. a typo alias
  // resolving to the same name as an already-listed item) instead of saving
  // them as separate rows on the same kill. This is a duplicate *entry* of
  // the same drop, not a second drop -- confirmed for real when a Balthazard
  // kill that actually dropped 4 "...Fragment" also had a typo'd "...Fragmer"
  // x1 row, and summing them (5) overcounted the actual loot. The first
  // occurrence's quantity wins; a later duplicate only fills in whichever of
  // crows/diamonds/sold the first one was missing, rather than adding to it.
  ...
  for (const item of cleaned) {
    ...
    if (existing.crowsValue === null && item.crowsValue !== null) existing.crowsValue = item.crowsValue;
    if (existing.diamondsValue === null && item.diamondsValue !== null) existing.diamondsValue = item.diamondsValue;
    if (existing.soldQuantity === 0 && item.soldQuantity > 0) existing.soldQuantity = item.soldQuantity;
    existing.sold = existing.soldQuantity >= existing.quantity;
  }
  return Array.from(byName.values());
}

// Returns un-awaited query promises (not executed yet) for the caller to run
// together via sqlTransaction -- see the DELETE-then-reinsert callers below,
// where inserting one-by-one in a plain loop meant a single bad row (e.g. a
// value Postgres rejects) left the already-deleted old rows gone with only
// some of the new ones written back, silently losing whichever items never
// got to run. Bundled in one transaction, a failure anywhere rolls back the
// whole batch instead of partially applying it.
function buildLootItemInserts(attendanceId, lootItems) {
  return lootItems.map(
    (item) => sql`
      INSERT INTO world_boss_loot_items (id, attendance_id, item_name, quantity, crows_value, diamonds_value, sold, sold_quantity)
      VALUES (${crypto.randomUUID()}, ${attendanceId}, ${item.itemName}, ${item.quantity}, ${item.crowsValue}, ${item.diamondsValue}, ${item.sold}, ${item.soldQuantity})
    `
  );
}

// Every bonus row on one event always carries the same admin-entered
// points value -- there's no per-attendee amount, just who was there when
// the fight was lost.
function buildBonusPointInserts(attendanceId, memberNames, points) {
  return memberNames.map(
    (name) => sql`INSERT INTO world_boss_bonus_points (id, attendance_id, member_name, points) VALUES (${crypto.randomUUID()}, ${attendanceId}, ${name}, ${points})`
  );
}

// Newest event first. Extra queries for all attendees/loot/bonus points
// (rather than N+1 per event) since a guild's history can run to hundreds
// of events.
app.get('/api/world-boss-attendance', async (req, res) => {
  ...
  const { rows: bonusRows } = await sql`SELECT * FROM world_boss_bonus_points`;
  ...
  bonusRows.forEach((b) => {
    if (!bonusByEvent.has(b.attendance_id)) bonusByEvent.set(b.attendance_id, []);
    bonusByEvent.get(b.attendance_id).push(b);
  });
  res.json(events.map((e) => serializeWorldBossEvent(e, attendeesByEvent.get(e.id) || [], lootByEvent.get(e.id) || [], bonusByEvent.get(e.id) || [])));
});

// Looks up each attendee's current guild from Growth Rate submissions at
// record time -- denormalized onto world_boss_attendees so a member's later
// guild change (or removal from Growth Rate) doesn't rewrite this history.
// Was sovereign_members, but that table is missing/misaligned guilds (e.g.
// no BUBBLEGANG) for who actually attends -- Growth Rate submissions are
// the roster of record everywhere attendance is tracked.
async function resolveAttendeeGuilds(names) {
  if (!names.length) return new Map();
  const { rows } = await sql`SELECT DISTINCT ON (LOWER(ign)) ign, guild_name FROM growth_submissions WHERE LOWER(ign) = ANY(${names.map((n) => n.toLowerCase())}) ORDER BY LOWER(ign), updated_at DESC`;
  const byLowerName = new Map(rows.map((r) => [r.ign.toLowerCase(), r.guild_name]));
  return new Map(names.map((n) => [n, byLowerName.get(n.toLowerCase()) || null]));
}

// One-time cleanup for attendee rows saved before resolveAttendeeGuilds
// pointed at Growth Rate submissions (it used to query the now-removed
// sovereign_members table, which was missing/misaligned guilds) -- those
// rows are stuck with a null guild_name forever since it's denormalized at
// record time. Re-resolves every such name against current Growth Rate
// data and fills in whatever now matches.
app.post('/api/world-boss-attendance/backfill-guilds', async (req, res) => {
  const { rows: missing } = await sql`SELECT DISTINCT member_name FROM world_boss_attendees WHERE guild_name IS NULL`;
  if (!missing.length) return res.json({ updated: 0 });
  const names = missing.map((m) => m.member_name);
  const guildByName = await resolveAttendeeGuilds(names);
  let updated = 0;
  for (const name of names) {
    const guild = guildByName.get(name);
    if (!guild) continue;
    const { rowCount } = await sql`UPDATE world_boss_attendees SET guild_name = ${guild} WHERE member_name = ${name} AND guild_name IS NULL`;
    updated += rowCount;
  }
  await logActivity(req, {
    action: 'update',
    entityType: 'world_boss_attendees',
    entityId: 'backfill',
    description: `Backfilled guild names for ${updated} attendee record${updated === 1 ? '' : 's'} from Growth Rate submissions`,
  });
  res.json({ updated });
});

app.post('/api/world-boss-attendance', async (req, res) => {
  const { bossName, eventDate, attendeeNames, lootItems, result, bonusPoints } = req.body || {};
  const schedule = WORLD_BOSS_SCHEDULES[req.body?.schedule] ? req.body.schedule : 'world_boss';
  const allowedBossNames = bossNamesForSchedule(schedule);
  if (!bossName || !allowedBossNames.includes(bossName)) {
    return res.status(400).json({ error: `bossName must be one of: ${allowedBossNames.join(', ')}` });
  }
  ...
  const resultValue = ['win', 'lose', 'pending'].includes(result) ? result : 'pending';
  const bonusPointsNum = bonusPoints === '' || bonusPoints === null || bonusPoints === undefined ? null : Number(bonusPoints);
  if (bonusPointsNum !== null && !Number.isFinite(bonusPointsNum)) return res.status(400).json({ error: 'bonusPoints must be a number' });
  // Bonus points (and Points/Salary, which are built on them) are a World
  // Boss concept only -- BF4 Boss never gets any, even if sent.
  const hasBonus = schedule === 'world_boss' && resultValue !== 'pending' && bonusPointsNum !== null && bonusPointsNum > 0;

  const id = crypto.randomUUID();
  await sql`INSERT INTO world_boss_attendance (id, boss_name, event_date, schedule, result) VALUES (${id}, ${bossName}, ${eventDate}, ${schedule}, ${resultValue})`;
  const guildByName = await resolveAttendeeGuilds(names);
  for (const name of names) {
  ...
  if (loot.length) await sqlTransaction(buildLootItemInserts(id, loot));
  if (hasBonus) await sqlTransaction(buildBonusPointInserts(id, names, bonusPointsNum));
  ...
  const { rows: bonusRows } = await sql`SELECT * FROM world_boss_bonus_points WHERE attendance_id = ${id}`;
  await logActivity(req, {
    action: 'create',
    entityType: 'world_boss_attendance',
    entityId: id,
    description: `Logged ${names.length} attendee${names.length === 1 ? '' : 's'} for ${bossName} on ${eventDate}`,
    after: { bossName, eventDate, attendees: names, lootItems: loot, result: resultValue, bonusPoints: hasBonus ? bonusPointsNum : null },
  });
  res.status(201).json(serializeWorldBossEvent(rows[0], attendees, lootRows, bonusRows));
});
...
app.put('/api/world-boss-attendance/:id', async (req, res) => {
  ...
  const { rows: existingBonusRows } = await sql`SELECT member_name, points FROM world_boss_bonus_points WHERE attendance_id = ${req.params.id}`;
  const { bossName, eventDate, attendeeNames, lootItems, result, bonusPoints } = req.body || {};
  const allowedBossNames = bossNamesForSchedule(existing.schedule);
  let nextBoss = existing.boss_name;
  let nextDate = existing.event_date;
  if (bossName !== undefined) {
    if (!allowedBossNames.includes(bossName)) return res.status(400).json({ error: `bossName must be one of: ${allowedBossNames.join(', ')}` });
    nextBoss = bossName;
  }
  ...
  let nextResult = existing.result || 'pending';
  if (result !== undefined) {
    if (!['win', 'lose', 'pending'].includes(result)) return res.status(400).json({ error: 'result must be one of: win, lose, pending' });
    nextResult = result;
  }
  let nextBonusPoints = existingBonusRows.length ? Number(existingBonusRows[0].points) : null;
  if (bonusPoints !== undefined) {
    nextBonusPoints = bonusPoints === '' || bonusPoints === null ? null : Number(bonusPoints);
    if (nextBonusPoints !== null && !Number.isFinite(nextBonusPoints)) return res.status(400).json({ error: 'bonusPoints must be a number' });
  }
  // Bonus points are a World Boss concept only -- see the POST route above.
  const nextHasBonus = existing.schedule === 'world_boss' && nextResult !== 'pending' && nextBonusPoints !== null && nextBonusPoints > 0;
  // Attendee list, result, or amount changing all affect who should have a
  // bonus row (or whether any should exist at all) -- recompute from
  // scratch rather than trying to diff against the old set.
  const bonusRelevantChange = attendeeNames !== undefined || result !== undefined || bonusPoints !== undefined;

  await sql`UPDATE world_boss_attendance SET boss_name = ${nextBoss}, event_date = ${nextDate}, result = ${nextResult} WHERE id = ${req.params.id}`;
  if (attendeeNames !== undefined) {
    const guildByName = await resolveAttendeeGuilds(nextNames);
    await sqlTransaction([
      sql`DELETE FROM world_boss_attendees WHERE attendance_id = ${req.params.id}`,
      ...nextNames.map(
        (name) => sql`INSERT INTO world_boss_attendees (id, attendance_id, member_name, guild_name) VALUES (${crypto.randomUUID()}, ${req.params.id}, ${name}, ${guildByName.get(name) || null})`
      ),
    ]);
  }
  if (nextLoot !== null) {
    await sqlTransaction([
      sql`DELETE FROM world_boss_loot_items WHERE attendance_id = ${req.params.id}`,
      ...buildLootItemInserts(req.params.id, nextLoot),
    ]);
  }
  if (bonusRelevantChange) {
    await sqlTransaction([
      sql`DELETE FROM world_boss_bonus_points WHERE attendance_id = ${req.params.id}`,
      ...(nextHasBonus ? buildBonusPointInserts(req.params.id, nextNames, nextBonusPoints) : []),
    ]);
  }
  ...
  const { rows: bonusRows } = await sql`SELECT * FROM world_boss_bonus_points WHERE attendance_id = ${req.params.id}`;
  await logActivity(req, {
    action: 'update',
    entityType: 'world_boss_attendance',
    entityId: req.params.id,
    ...
    before: {
      bossName: existing.boss_name,
      eventDate: existing.event_date,
      attendees: existingAttendees.map((a) => a.member_name),
      lootItems: existingLoot,
      result: existing.result || 'pending',
      bonusPoints: existingBonusRows.length ? Number(existingBonusRows[0].points) : null,
    },
    after: {
      bossName: nextBoss,
      eventDate: nextDate,
      attendees: nextNames,
      lootItems: nextLoot !== null ? nextLoot : existingLoot,
      result: nextResult,
      bonusPoints: nextHasBonus ? nextBonusPoints : null,
    },
  });
  res.json(serializeWorldBossEvent(rows[0], attendees, lootRows, bonusRows));
});
...
// Ranked totals across every World Boss event ever logged -- BF4 Boss is
// deliberately excluded (a separate, higher-gear roster that Points/Salary
// aren't meant to factor in), regardless of whether the bonus came from a
// win or a loss. Grouped in SQL rather than summed client-side since the
// raw rows could grow large over time.
app.get('/api/world-boss-bonus-points/leaderboard', async (req, res) => {
  const { rows } = await sql`
    SELECT b.member_name, SUM(b.points) AS total_points, COUNT(DISTINCT b.attendance_id) AS event_count
    FROM world_boss_bonus_points b
    JOIN world_boss_attendance a ON a.id = b.attendance_id
    WHERE COALESCE(a.schedule, 'world_boss') = 'world_boss'
    GROUP BY b.member_name
    ORDER BY total_points DESC, LOWER(b.member_name) ASC
  `;
  res.json(rows.map((r) => ({ name: r.member_name, points: Number(r.total_points), eventCount: Number(r.event_count) })));
});
...
app.put('/api/world-boss-attendance/loot-items/:itemId/sold', async (req, res) => {
  const { sold } = req.body || {};
  // A manual toggle is always all-or-nothing -- full quantity or 0, never a
  // partial (partial only ever comes from FIFO-matching against sales, see
  // applyFifoSalesToItem).
  const { rows } = await sql`
    UPDATE world_boss_loot_items SET sold = ${sold === true}, sold_quantity = CASE WHEN ${sold === true} THEN quantity ELSE 0 END
    WHERE id = ${req.params.itemId}
    RETURNING id, item_name, sold, sold_quantity
  `;
  ...
  res.json({ id: rows[0].id, sold: rows[0].sold === true, soldQuantity: Number(rows[0].sold_quantity) });
});

function serializeManagementFee(row) {
  return { id: row.id, ign: row.ign, percent: Number(row.percent) };
}

// Standing accounting/management cuts for the World Boss salary
// computation -- small reference table, not month-scoped (see
// world_boss_management_fees in lib/db.js for why).
app.get('/api/world-boss-management-fees', async (req, res) => {
  const { rows } = await sql`SELECT * FROM world_boss_management_fees ORDER BY LOWER(ign) ASC`;
  res.json(rows.map(serializeManagementFee));
});

app.post('/api/world-boss-management-fees', async (req, res) => {
  const { ign, percent } = req.body || {};
  const trimmed = (ign || '').trim();
  if (!trimmed) return res.status(400).json({ error: 'ign is required' });
  const percentNum = Number(percent);
  if (!Number.isFinite(percentNum) || percentNum <= 0 || percentNum > 100) return res.status(400).json({ error: 'percent must be between 0 and 100' });
  const { rows: existing } = await sql`SELECT id FROM world_boss_management_fees WHERE LOWER(ign) = LOWER(${trimmed})`;
  if (existing.length) return res.status(409).json({ error: `${trimmed} already has a management fee -- remove it first to change the percent` });
  const id = crypto.randomUUID();
  const { rows } = await sql`INSERT INTO world_boss_management_fees (id, ign, percent) VALUES (${id}, ${trimmed}, ${percentNum}) RETURNING *`;
  await logActivity(req, {
    action: 'create',
    entityType: 'world_boss_management_fee',
    entityId: id,
    description: `Added a ${percentNum}% management fee for "${trimmed}"`,
    after: { ign: trimmed, percent: percentNum },
  });
  res.status(201).json(serializeManagementFee(rows[0]));
});

app.delete('/api/world-boss-management-fees/:id', async (req, res) => {
  const { rows } = await sql`DELETE FROM world_boss_management_fees WHERE id = ${req.params.id} RETURNING *`;
  if (!rows.length) return res.status(404).json({ error: 'Not found' });
  await logActivity(req, {
    action: 'delete',
    entityType: 'world_boss_management_fee',
    entityId: req.params.id,
    description: `Removed the management fee for "${rows[0].ign}"`,
    before: { ign: rows[0].ign, percent: Number(rows[0].percent) },
  });
  res.status(204).end();
});
...
app.get('/api/loot-item-sources', async (req, res) => {
  const { rows } = await sql`SELECT item_key, item_name, boss_name, schedule FROM loot_item_sources`;
  res.json(rows.map((r) => ({ itemKey: r.item_key, itemName: r.item_name, bossName: r.boss_name, schedule: r.schedule || 'world_boss' })));
});
...
app.put('/api/loot-item-sources', async (req, res) => {
  ...
  const allowedBossNames = bossNamesForSchedule(schedule);
  if (bossName !== null && bossName !== undefined && !allowedBossNames.includes(bossName)) {
    return res.status(400).json({ error: `bossName must be one of: ${allowedBossNames.join(', ')}` });
  }
  await sql`
    INSERT INTO loot_item_sources (item_key, item_name, boss_name, schedule, updated_at)
    VALUES (${itemKey}, ${String(itemName).trim()}, ${bossName || null}, ${schedule}, now())
    ON CONFLICT (item_key, schedule) DO UPDATE SET item_name = EXCLUDED.item_name, boss_name = EXCLUDED.boss_name, updated_at = now()
  `;
  await logActivity(req, {
    action: 'update',
...
function serializeSaleBatch(row) {
  return {
    id: row.id,
    ...
    crowsValue: row.crows_value === null ? null : Number(row.crows_value),
    diamondsValue: row.diamonds_value === null ? null : Number(row.diamonds_value),
    soldAt: row.sold_at,
...
// One row per sale, independent of which kill(s) the sold pieces dropped
// from -- an item's Total Quantity is the sum of every drop across all
// kills (computed client-side from the events already loaded), and each
// batch here just records "N of them sold for this much, on this date".
app.get('/api/loot-sale-batches', async (req, res) => {
  const { rows } = await sql`SELECT * FROM loot_sale_batches ORDER BY sold_at DESC`;
  res.json(rows.map(serializeSaleBatch));
});

app.post('/api/loot-sale-batches', async (req, res) => {
  const { itemName, quantity, crowsValue, diamondsValue, soldAt, sourceAttendanceId } = req.body || {};
  const schedule = WORLD_BOSS_SCHEDULES[req.body?.schedule] ? req.body.schedule : 'world_boss';
  const itemKey = lootItemKey(itemName);
  const quantityNum = Number(quantity);
  if (!itemKey) return res.status(400).json({ error: 'itemName is required' });
  if (!Number.isFinite(quantityNum) || quantityNum <= 0) return res.status(400).json({ error: 'quantity must be a positive number' });
  const crows = crowsValue === '' || crowsValue === null || crowsValue === undefined ? null : Number(crowsValue);
  const diamonds = diamondsValue === '' || diamondsValue === null || diamondsValue === undefined ? null : Number(diamondsValue);
  if (crows !== null && !Number.isFinite(crows)) return res.status(400).json({ error: 'crowsValue must be a number' });
  if (diamonds !== null && !Number.isFinite(diamonds)) return res.status(400).json({ error: 'diamondsValue must be a number' });
  ...
  const { rows } = await sql`
    ...
    VALUES (${id}, ${itemKey}, ${String(itemName).trim()}, ${schedule}, ${quantityNum}, ${crows}, ${diamonds}, ${soldAt ? new Date(soldAt) : new Date()}, ${req.currentUser.username}, ${sourceAttendanceId || null})
    RETURNING *
  `;
  await logActivity(req, {
    action: 'create',
    entityType: 'loot_sale_batch',
    entityId: id,
    description: `Sold ${quantityNum} of "${itemName}"`,
    after: { itemName, quantity: quantityNum, crowsValue: crows, diamondsValue: diamonds },
  });
  res.status(201).json(serializeSaleBatch(rows[0]));
});

app.delete('/api/loot-sale-batches/:id', async (req, res) => {
  const { rows } = await sql`DELETE FROM loot_sale_batches WHERE id = ${req.params.id} RETURNING *`;
  if (!rows.length) return res.status(404).json({ error: 'Not found' });
  await logActivity(req, {
    action: 'delete',
    entityType: 'loot_sale_batch',
    entityId: req.params.id,
    description: `Removed a sale of ${Number(rows[0].quantity)} "${rows[0].item_name}"`,
    before: { quantity: Number(rows[0].quantity), crowsValue: rows[0].crows_value, diamondsValue: rows[0].diamonds_value },
  });
  res.status(204).end();
});
...
// Catches whatever the per-route async wrapper above forwards -- always
// last, and takes 4 args so Express recognizes it as an error handler.
