...
function normalizeLootItems(lootItems) {
  ...
  const cleaned = lootItems
    .map((l) => ({
      ...
      // Both are a whole-number currency column (BIGINT) -- a stray decimal
      // (e.g. a typo'd "33.25") would otherwise reach Postgres as-is and get
      // rejected outright, failing the whole save with no clear feedback.
      crowsValue: l?.crowsValue === '' || l?.crowsValue === null || l?.crowsValue === undefined ? null : Math.round(Number(l.crowsValue)),
      diamondsValue: l?.diamondsValue === '' || l?.diamondsValue === null || l?.diamondsValue === undefined ? null : Math.round(Number(l.diamondsValue)),
      sold: l?.sold === true,
    }))
    .filter((l) => l.itemName);
