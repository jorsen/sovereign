...
app.post('/api/world-boss-attendance', async (req, res) => {
  const { bossName, eventDate, attendeeNames } = req.body || {};
  ...
  await logActivity(req, {
    ...
    after: { bossName, eventDate, attendees: names },
  });
...
app.put('/api/world-boss-attendance/:id', async (req, res) => {
  ...
  await logActivity(req, {
    ...
    before: { bossName: existing.boss_name, eventDate: existing.event_date, attendees: existingAttendees.map((a) => a.member_name) },
    after: { bossName: nextBoss, eventDate: nextDate, attendees: nextNames },
  });
