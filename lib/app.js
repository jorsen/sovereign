...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  // Matched primarily by Discord account -- the same real person can typo
  // or re-spell their IGN between submissions (e.g. "Bazo" vs "Bazô"), but
  // discord_user_id never changes, so it catches that case an exact IGN
  // match alone would miss. IGN match stays as a fallback for the rare case
  // discordUserId is missing.
  const { rows: existingByIgn } = await sql`
    SELECT id FROM growth_submissions
    WHERE discord_user_id = ${discordUserId} OR LOWER(ign) = LOWER(${ignTrimmed})
    ORDER BY (discord_user_id = ${discordUserId}) DESC
    LIMIT 1
  `;
