...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  // Same IGN re-submitting (a typo fixed, stats grown, whatever) updates
  // their one existing row instead of piling up a new one every time --
  // matched case-insensitively since Discord doesn't enforce IGN casing.
...
async function insertLootItems(attendanceId, lootItems) {
  for (const item of lootItems) {
    await sql`
      INSERT INTO world_boss_loot_items (id, attendance_id, item_name, quantity, crows_value, diamonds_value)
      VALUES (${crypto.randomUUID()}, ${attendanceId}, ${item.itemName}, ${item.quantity}, ${item.crowsValue}, ${item.diamondsValue})
    `;
  }
}
