...
// row that used the old name, so past rosters reflect the correction instead
// of the next participant save (upsertSovereignMember) re-creating the old
// name from a stale roster row and fighting the rename.
app.put('/api/sovereign-members/:id', async (req, res) => {
  ...
  `;
  await sql`UPDATE crusade_participants SET name = ${trimmed} WHERE LOWER(name) = LOWER(${existing.name})`;
