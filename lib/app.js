...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  const { rows } = await sql`
    ...
    VALUES (${id}, ${discordMessageId}, ${discordUserId}, ${discordUsername || null}, ${ign.trim()}, ${className.trim()}, ${guildName}, ${lampLevelNum}, ${growthRateNum}, ${imageBuffer}, ${imageContentType})
...
function serializeWorldBossEvent(row, attendees) {
  return {
    id: row.id,
...
// Newest event first. One extra query for all attendees (rather than N+1
// per event) since a guild's history can run to hundreds of events.
app.get('/api/world-boss-attendance', async (req, res) => {
  ...
  res.json(events.map((e) => serializeWorldBossEvent(e, attendeesByEvent.get(e.id) || [])));
});
...
app.post('/api/world-boss-attendance', async (req, res) => {
  const { bossName, eventDate, attendeeNames, loot } = req.body || {};
  ...
  const id = crypto.randomUUID();
  await sql`INSERT INTO world_boss_attendance (id, boss_name, event_date, loot) VALUES (${id}, ${bossName}, ${eventDate}, ${loot ? String(loot).trim() : null})`;
  ...
  await logActivity(req, {
    ...
    after: { bossName, eventDate, attendees: names, loot: loot || null },
  });
  res.status(201).json(serializeWorldBossEvent(rows[0], attendees));
});
...
app.put('/api/world-boss-attendance/:id', async (req, res) => {
  ...
  if (loot !== undefined) nextLoot = loot ? String(loot).trim() : null;
  ...
  await sql`UPDATE world_boss_attendance SET boss_name = ${nextBoss}, event_date = ${nextDate}, loot = ${nextLoot} WHERE id = ${req.params.id}`;
  if (attendeeNames !== undefined) {
  ...
  await logActivity(req, {
    ...
    before: { bossName: existing.boss_name, eventDate: existing.event_date, attendees: existingAttendees.map((a) => a.member_name), loot: existing.loot },
    after: { bossName: nextBoss, eventDate: nextDate, attendees: nextNames, loot: nextLoot },
  });
  res.json(serializeWorldBossEvent(rows[0], attendees));
});
