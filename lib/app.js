...
app.post('/api/world-boss-attendance', async (req, res) => {
  ...
  const hasBonus = resultValue !== 'pending' && bonusPointsNum !== null && bonusPointsNum > 0;
...
app.put('/api/world-boss-attendance/:id', async (req, res) => {
  ...
  const nextHasBonus = nextResult !== 'pending' && nextBonusPoints !== null && nextBonusPoints > 0;
...
// Ranked totals across every World Boss/BF4 event ever logged, regardless
// of schedule or whether the bonus came from a win or a loss. Grouped in
// SQL rather than summed client-side since the raw rows could grow large
// over time.
app.get('/api/world-boss-bonus-points/leaderboard', async (req, res) => {
  const { rows } = await sql`
    SELECT member_name, SUM(points) AS total_points, COUNT(DISTINCT attendance_id) AS event_count
    FROM world_boss_bonus_points
    GROUP BY member_name
    ORDER BY total_points DESC, LOWER(member_name) ASC
  `;
