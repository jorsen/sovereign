...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  // Total distinct IGNs currently on the Growth Rate page -- the bot uses
  // this to tell an admin channel "X total submissions now" alongside the
  // save confirmation, without a second round-trip to compute it itself.
  const { rows: countRows } = await sql`SELECT COUNT(*)::int AS count FROM growth_submissions`;
  res.status(201).json({ ...serializeGrowthSubmission(rows[0]), totalSubmissions: countRows[0].count, isNewSubmission: !isUpdate });
});
