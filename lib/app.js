...
function normalizeLootItems(lootItems) {
  ...
  const cleaned = lootItems
    .map((l) => ({
      ...
      // NUMERIC columns -- decimals allowed (e.g. splitting one sale's total
      // proportionally across several kills), just guarded against garbage
      // input reaching Postgres as a bad parameter.
  ...
  for (const item of cleaned) {
    ...
    existing.sold = existing.sold || item.sold;
  }
...
app.post('/api/loot-sale-batches', async (req, res) => {
  const { itemName, quantity, crowsValue, diamondsValue, soldAt } = req.body || {};
  const schedule = WORLD_BOSS_SCHEDULES[req.body?.schedule] ? req.body.schedule : 'world_boss';
  const itemKey = lootItemKey(itemName);
  ...
  const id = crypto.randomUUID();
  const { rows } = await sql`
    INSERT INTO loot_sale_batches (id, item_key, item_name, schedule, quantity, crows_value, diamonds_value, sold_at, created_by)
    VALUES (${id}, ${itemKey}, ${String(itemName).trim()}, ${schedule}, ${quantityNum}, ${crows}, ${diamonds}, ${soldAt ? new Date(soldAt) : new Date()}, ${req.currentUser.username})
    RETURNING *
  `;
  await logActivity(req, {
    action: 'create',
