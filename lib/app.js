...
app.post('/api/world-boss-attendance', async (req, res) => {
  ...
  // Bonus points (and Points/Salary, which are built on them) apply to
  // every schedule except BF4 Boss, which never gets any even if sent.
  const hasBonus = schedule !== 'bf4' && resultValue !== 'pending' && bonusPointsNum !== null && bonusPointsNum > 0;
...
app.put('/api/world-boss-attendance/:id', async (req, res) => {
  ...
  // Bonus points apply to every schedule except BF4 -- see the POST route above.
  const nextHasBonus = existing.schedule !== 'bf4' && nextResult !== 'pending' && nextBonusPoints !== null && nextBonusPoints > 0;
...
// Ranked totals across every non-BF4 schedule ever logged -- BF4 Boss is
...
app.get('/api/world-boss-bonus-points/leaderboard', async (req, res) => {
  const { rows } = await sql`
    ...
    WHERE COALESCE(a.schedule, 'world_boss') != 'bf4'
