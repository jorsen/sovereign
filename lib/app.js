...
app.post('/api/world-boss-attendance', async (req, res) => {
  ...
  const hasBonus = resultValue === 'lose' && bonusPointsNum !== null && bonusPointsNum > 0;
...
app.put('/api/world-boss-attendance/:id', async (req, res) => {
  ...
  const nextHasBonus = nextResult === 'lose' && nextBonusPoints !== null && nextBonusPoints > 0;
...
// of schedule -- a bonus for showing up to a loss counts the same either
// way. Grouped in SQL rather than summed client-side since the raw rows
