...
app.get('/api/growth-submissions', async (req, res) => {
  const { rows } = await sql`SELECT id, ign, class, discord_username, image_content_type, created_at, updated_at FROM growth_submissions ORDER BY created_at DESC`;
...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  const { discordMessageId, discordUserId, discordUsername, ign, class: className, imageBase64, imageContentType } = req.body || {};
  if (!discordMessageId || !discordUserId || !ign || !className || !imageBase64 || !imageContentType) {
    return res.status(400).json({ error: 'discordMessageId, discordUserId, ign, class, imageBase64, and imageContentType are all required' });
  }
  ...
  const id = crypto.randomUUID();
  const { rows } = await sql`
    INSERT INTO growth_submissions (id, discord_message_id, discord_user_id, discord_username, ign, class, image_data, image_content_type)
    VALUES (${id}, ${discordMessageId}, ${discordUserId}, ${discordUsername || null}, ${ign.trim()}, ${className.trim()}, ${imageBuffer}, ${imageContentType})
    ON CONFLICT (discord_message_id) DO UPDATE SET
      ...
      updated_at = now()
    RETURNING id, ign, class, discord_username, image_content_type, created_at, updated_at
  `;
