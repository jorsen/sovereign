...
app.post('/api/loot-sale-batches', async (req, res) => {
  ...
  const id = crypto.randomUUID();
  const { rows } = await sql`
    INSERT INTO loot_sale_batches (id, item_key, item_name, quantity, crows_value, diamonds_value, sold_at, created_by)
    VALUES (${id}, ${itemKey}, ${String(itemName).trim()}, ${quantityNum}, ${crows}, ${diamonds}, ${soldAt ? new Date(soldAt) : new Date()}, ${req.currentUser.username})
    RETURNING *
  `;
  await logActivity(req, {
    action: 'create',
