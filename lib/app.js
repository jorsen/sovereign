...
const GROWTH_SUBMISSION_GUILDS = ['Helloシ', '貓貓客棧', '巫女組', 'CAPITAL'];
...
// Newest first, no image bytes -- the web page fetches each screenshot
// separately via GET .../:id/image so the list itself stays small.
app.get('/api/growth-submissions', async (req, res) => {
  const { rows } = await sql`SELECT id, ign, class, guild_name, lamp_level, discord_username, image_content_type, created_at, updated_at FROM growth_submissions ORDER BY created_at DESC`;
...
app.post('/api/growth-submissions/bot', async (req, res) => {
  ...
  const { discordMessageId, discordUserId, discordUsername, ign, class: className, guildName, lampLevel, imageBase64, imageContentType } = req.body || {};
  if (!discordMessageId || !discordUserId || !ign || !className || !guildName || lampLevel === undefined || lampLevel === null || !imageBase64 || !imageContentType) {
    return res.status(400).json({ error: 'discordMessageId, discordUserId, ign, class, guildName, lampLevel, imageBase64, and imageContentType are all required' });
  }
  ...
  if (!Number.isInteger(lampLevelNum) || lampLevelNum < 0) {
    return res.status(400).json({ error: 'lampLevel must be a non-negative integer' });
  }
  ...
  const id = crypto.randomUUID();
  const { rows } = await sql`
    INSERT INTO growth_submissions (id, discord_message_id, discord_user_id, discord_username, ign, class, guild_name, lamp_level, image_data, image_content_type)
    VALUES (${id}, ${discordMessageId}, ${discordUserId}, ${discordUsername || null}, ${ign.trim()}, ${className.trim()}, ${guildName}, ${lampLevelNum}, ${imageBuffer}, ${imageContentType})
    ON CONFLICT (discord_message_id) DO UPDATE SET
      discord_username = EXCLUDED.discord_username,
      ign = EXCLUDED.ign,
      class = EXCLUDED.class,
      guild_name = EXCLUDED.guild_name,
      lamp_level = EXCLUDED.lamp_level,
      image_data = EXCLUDED.image_data,
      image_content_type = EXCLUDED.image_content_type,
      updated_at = now()
    RETURNING id, ign, class, guild_name, lamp_level, discord_username, image_content_type, created_at, updated_at
  `;
...
app.put('/api/growth-submissions/:id', async (req, res) => {
  ...
  const { ign, class: className, guildName, lampLevel } = req.body || {};
  ...
  if (lampLevel !== undefined) {
    ...
    if (!Number.isInteger(n) || n < 0) return res.status(400).json({ error: 'lampLevel must be a non-negative integer' });
    nextLamp = n;
  }

  const { rows } = await sql`
    UPDATE growth_submissions SET ign = ${nextIgn}, class = ${nextClass}, guild_name = ${nextGuild}, lamp_level = ${nextLamp}, updated_at = now()
    WHERE id = ${req.params.id}
    RETURNING id, ign, class, guild_name, lamp_level, discord_username, image_content_type, created_at, updated_at
  `;
  await logActivity(req, {
    ...
    before: { ign: existing.ign, class: existing.class, guildName: existing.guild_name, lampLevel: existing.lamp_level },
    after: { ign: nextIgn, class: nextClass, guildName: nextGuild, lampLevel: nextLamp },
  });
