...
function normalizeLootItems(lootItems) {
  ...
  // Merge same-name rows within this one submission (e.g. a typo alias
  // resolving to the same name as an already-listed item) into one, instead
  // of saving them as separate rows on the same kill.
  ...
  for (const item of cleaned) {
    ...
    if (item.crowsValue !== null) existing.crowsValue = (existing.crowsValue || 0) + item.crowsValue;
    if (item.diamondsValue !== null) existing.diamondsValue = (existing.diamondsValue || 0) + item.diamondsValue;
