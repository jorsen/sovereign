...
// Posts the PUBLIC confirmation message -- explicitly no `flags`, so it's
// visible to everyone in the channel regardless of the original deferred
// response being ephemeral. Logs (rather than silently swallowing) a
// failure here, since this is the one part of the flow other members
// actually see -- a silent failure here previously looked to the submitter
// like everything worked (their own ephemeral "Saved" still showed) while
// nothing public ever appeared.
async function postFollowup(applicationId, token, payload) {
  const res = await fetchWithRetry(`https://discord.com/api/v10/webhooks/${applicationId}/${token}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ...payload, flags: 0 }),
  });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    console.error('postFollowup failed:', res.status, body);
