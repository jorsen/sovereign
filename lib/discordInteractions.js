...
async function postPublicConfirmation(channelId, payload) {
  ...
  if (!res.ok) {
    ...
    console.error('postPublicConfirmation failed:', res.status, body);
...
async function processUniongrSubmission({ applicationId, token, interactionId, channelId, siteBaseUrl, options, resolved, member, user }) {
  ...
  try {
    ...
    await postPublicConfirmation(channelId, {
      embeds: [
        {
          title: ign,
          fields: [
