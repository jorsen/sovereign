...
// on it) or an error -- nothing about a submission is ever posted publicly
// to the channel.
//
...
async function processUniongrSubmission({ applicationId, token, interactionId, channelId, siteBaseUrl, options, resolved, member, user }) {
  ...
  try {
    ...
    // Everything about this submission stays private to the submitter --
    // no public post to the channel. The private reply itself says it was
    // saved, and a ✅ reaction on that same message gives them an
    // immediately-visible confirmation without needing to read the text.
